...
function keystone_setup(){
    ...
    sed -i "s|^\[cache]|[cache]\nenable = true \nbackend = dogpile.cache.memcached \nbackend_argument = url:$MEMCACHED_HOST:$MEMCACHED_HOST |g" /etc/keystone/keystone.conf
    check_permissions
...
function populate_keystone(){
    ...
    # openstack user list --os-token $ryans_token --os-url $KEYSTONE_INTERNAL_ENDPOINT --os-identity-api-version 3
  }
